...
export default function ActorDashboard() {
  ...
  const handleSendOffer = async (e: React.FormEvent) => {
    ...
    setOfferForm({ amount: '', advance: '', timeline: '', dueDate: '', approach: '', deliverables: '', additionalDetails: '' });
    setInfoModal({ isOpen: true, title: 'Success', content: 'Offer submitted successfully' });
  };
        ...
        >
          ...
          <div className="relative z-10 w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 p-[2px] shadow-[0_0_15px_rgba(59,130,246,0.3)] group-hover:shadow-[0_0_25px_rgba(59,130,246,0.5)] transition-shadow duration-300">
            <div className="w-full h-full rounded-full bg-slate-900 flex items-center justify-center overflow-hidden">
              ...
              <img src={getSmartImage("profiles")} alt="Profile" className="w-full h-full object-cover opacity-90" referrerPolicy="no-referrer" />
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-6">
              ...
              {activeTab === 'feed' ? (
                [...posts].sort((a, b) => (a.type === 'Need Post' ? -1 : 1)).map((post, i) => (
                  <motion.div 
        ...
        {sidebarMode === 'minimized' && (
          <div className="flex-1 p-4 flex flex-col gap-6 overflow-hidden">
            ...
            <div className="flex flex-col gap-2.5">
              <button onClick={() => setSidebarDetailModal({ type: 'projects', title: 'Suggested Projects For You', data: suggestedProjects })} className="w-full h-[40px] bg-slate-800/50 hover:bg-slate-700/50 border border-white/5 rounded-xl flex items-center justify-center text-sm font-medium text-slate-200 transition-colors">
            ...
            {/* JOIN GROUPS */}
            <div>
              <h3 className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-widest">Join Groups</h3>
              <div className="flex flex-col gap-3">
                {groups.slice(0, 2).map((group, i) => (
                  <div key={i} className="bg-slate-800/50 border border-white/5 rounded-xl p-3 hover:bg-slate-700/50 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center justify-between text-[10px]">
                      <span className="text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded">{group.members} Members</span>
                      <span className="text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded">Skill: {group.skill}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto scrollbar-hide">
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setSidebarDetailModal({ type: 'projects', title: 'Suggested Projects For You', data: suggestedProjects })} className="aspect-[4/3] bg-slate-800/50 hover:bg-slate-700/50 border border-white/5 rounded-xl flex flex-col items-center justify-center p-4 transition-colors group">
                    <span className="text-2xl font-bold text-white mb-1 group-hover:scale-110 transition-transform">{suggestedProjects.length}</span>
                    <span className="text-sm font-medium text-slate-300 text-center">Suggested<br/>Projects</span>
                  </button>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {groups.slice(0, 4).map((group, i) => (
                      <div key={i} className="bg-slate-800/50 border border-white/5 rounded-xl p-4 hover:bg-slate-700/50 transition-colors flex flex-col">
                        <h4 className="text-sm font-semibold text-white mb-1">{group.name}</h4>
                        <p className="text-xs text-slate-400 mb-3 flex-1">{group.desc}</p>
                        <div className="space-y-2 mb-4">
                          ...
                          <div className="flex items-center justify-between text-[10px]">
                            <span className="text-slate-400">Avg Skill Score</span>
                            <span className="text-purple-400 font-medium">{group.skill}</span>
                          </div>
                        </div>
                        <button className="w-full py-2 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 text-xs font-medium rounded-lg transition-colors border border-blue-500/20">
                          Join Group
                        </button>
                      </div>
                    ))}
                  ...
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-white/5">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-slate-800 overflow-hidden">
                        <img src={getSmartImage("profiles")} alt={actor.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      </div>
                      <div>
